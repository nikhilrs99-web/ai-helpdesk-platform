...
| Phase | Description |
|---|---|
| Phase 1: Setup | Scaffolded 	icket-service (Spring Boot 3.2, Java 21, JPA, PostgreSQL, REST controllers). Designed domain model with Strategy/State patterns for ticket routing and lifecycle management. Integrated Keycloak for stateless JWT authentication and role-based access control (Admin, Agent, Customer). Wrote 52 comprehensive unit and integration tests. |
| Phase 2: Core Microservices | Scaffolded kb-service with PostgreSQL full-text search (	svector), and a basic 
otification-service. Introduced Flyway for database migrations. Created pi-gateway (Spring Cloud Gateway) for centralized routing and JWT validation. Containerized all services using multi-stage Dockerfiles and wired them into docker-compose.yml. |
| Phase 3: Event-Driven Architecture | Replaced direct REST calls with a Transactional Outbox pattern writing safely to PostgreSQL, polled via OutboxWorker to publish idempotently to a KRaft Kafka cluster. 
otification-service and nalytics-service now consume these events robustly with DLQ and Retry configurations. Scheduled an SLA-breach cron job. |
| Phase 4: Caching & Rate Limiting | Spun up Redis within docker-compose. Applied @Cacheable annotations to kb-service for cache-aside reads. Built a custom RateLimiterService in 	icket-service to cap ticket creations per user, and an AgentPresenceService tracking online staff via TTL expirations. |
| Phase 5: Hybrid RAG Pipeline | Built i-service leveraging Spring AI. Initialized pgvector vector store with HNSW indexing. Implemented /ingest for generating Knowledge Base embeddings and /rag/search for semantic search and Retrieval-Augmented Generation. Added /ticket/analyze for auto-categorization and sentiment analysis. |
...
| Phase 8: Analytics & Frontend Kickoff | Created JPA schema and Kafka consumer to persist ticket metrics in nalytics-service. Built REST endpoints for dashboard SLAs and AI resolution rates. Scaffolded the React+Vite+Tailwind frontend SPA and wired the base routing. |
