...
| Day 18 | `notification-service` skeleton added: Implemented Observer-style notifier pattern (NotificationObserver, NotificationDispatcher) and a temporary direct REST endpoint (NotificationController) to act as the notification receiver before Kafka is introduced |
...
| Phase 3 Wrap-up | Completed Days 26-35 (Event-Driven Backbone): Replaced direct REST calls with a Transactional Outbox pattern writing safely to PostgreSQL (Days 26-27), polled via OutboxWorker to publish idempotently to a KRaft Kafka cluster (Day 28). notification-service and analytics-service now consume these events robustly with DLQ and Retry configurations (Days 29-32). Scheduled an SLA-breach cron job utilizing the outbox (Day 33), and verified with Testcontainers integration tests (Day 34-35). |
| Phase 4 Wrap-up | Completed Days 36-40 (Redis Integration): Spun up Redis within docker-compose (Day 36). Applied @Cacheable annotations to kb-service for cache-aside reads (Day 37). Built a custom RateLimiterService in ticket-service to cap ticket creations per user (Day 38) and an AgentPresenceService tracking online staff via TTL expirations (Day 39). Verified with Testcontainers Redis testing (Day 40). |
| Phase 5 Wrap-up | Completed Days 41-50 (Hybrid RAG Pipeline): Built ai-service leveraging Spring AI (Day 41, 47). Initialized pgvector vector_store with HNSW indexing (Day 42). Implemented /ingest for generating Knowledge Base embeddings (Day 43). Created /rag/search for semantic search (Day 44) and Retrieval-Augmented Generation (Day 45-46). Added /ticket/analyze for auto-categorization and sentiment analysis using LLMs (Day 48). Routed via api-gateway (Day 49) and added mock integration tests (Day 50). |
| Phase 6 Wrap-up | Completed Days 51-55 (AI Evaluation): Built a golden Q&A dataset (Day 51). Developed a custom evaluation harness (RagEvaluationTest) to measure Precision@K (Day 52) and LLM-as-a-judge for answer faithfulness (Day 53). Recorded baseline scores (Day 54). Tuned search parameters (TopK=5, similarityThreshold=0.75) based on eval feedback to reduce hallucinations (Day 55). |
| Phase 7 Wrap-up | Completed Days 56-60 (AI Agent Tool-Calling): Configured Spring AI function calling. Defined read-only tools: getTicketStatus, searchKnowledgeBase (Day 56) and getSLAStatus, getCustomerTickets (Day 57). Implemented a human-in-the-loop write tool: createEscalation (Day 58). Added unit tests asserting tool contracts (Day 59) and documented the Agentic workflow (Day 60). |
| Phase 8 Wrap-up | Completed Days 61-65 (Analytics + Frontend): Created JPA schema and Kafka consumer to persist ticket metrics in analytics-service (Day 61). Built REST endpoints for dashboard SLAs and AI resolution rates (Day 62). Scaffolded the React+Vite+Tailwind frontend SPA (Day 63) and wired the routing for Ticket Lists, Knowledge Base Search, and Analytics Dashboards (Day 64-65). |
| Phase 9 Wrap-up | Completed Days 66-70 (Frontend Completion): Added a responsive Recharts-powered analytics dashboard (Day 66). Built an AgentPresence component simulating real-time Redis pub/sub presence tracking (Day 67). Implemented polished Tailwind layout and responsive states (Day 68). Added a multi-stage Nginx Dockerfile for the frontend (Day 69) and successfully wired it into docker-compose.yml for a complete end-to-end stack (Day 70). |
| Phase 10 Wrap-up | Completed Days 71-80 (Kubernetes): Created an Umbrella Helm chart wrapping the entire platform architecture. Implemented Deployments, Services, ConfigMaps, and Secrets for all custom microservices and the frontend (Day 71-73). Added Bitnami subcharts for Postgres, Kafka, Redis, and Keycloak (Day 74). Added Horizontal Pod Autoscaling (HPA) for ai-service based on CPU utilization (Day 77) and configured liveness/readiness probes with resource requests/limits (Day 79). Fully documented the local Kind deployment process (Day 80). |
